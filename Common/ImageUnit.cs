...
namespace Common
{
   public class ImageUnit
    {
        private static LogUnit log = new LogUnit(typeof(ImageUnit));
        #region 缩略图
        /// <summary>
        /// 生成缩略图
        /// </summary>
        ...
        public static void MakeThumbnail(string originalImagePath, string thumbnailPath, int width, int height, string mode)
        {
            ...
            {
                log.Error("裁剪图片出错，原图片路径：" + originalImagePath + "，保存图片路径：" + thumbnailPath + "，异常", e);
                throw e;
            }
            finally
            {
        ...
        public static string ImageWatermark(string path, string waterpath, string location)
        {
            ...
            if (kz_name == ".jpg" || kz_name == ".bmp" || kz_name == ".jpeg")
            {
                ...
                string filename = EncryptUnit.CreatePasswordHash(path, 6);
        ...
        /// 添加一个图片水印
        /// </summary>
        /// <param name="orgImage">原图片</param>
        /// <param name="waterMarkImage">水印图片</param>
        /// <param name="location">位置</param>
        /// <returns></returns>
        //public static string ImageWaterMark(string orgImage, string waterMarkImage, WaterMarkLocation location)
        //{
        //    //创建一个包含照片水印的图像对象
        //    Image imgPhoto = Image.FromFile(orgImage);
        ...
        //    int[] loca = GetLoacation(imgPhoto, imgWatermark, location);
        //    int xPosOfWm = loca[0];
        //    int yPosOfWm = loca[1];
        //    //叠加
        ...
        //    string newFileName = Encrypt.CreatePasswordHash(orgImage, 6);
        //    string DictionaryPath = Path.GetDirectoryName(orgImage);
        //    string exten = Path.GetExtension(orgImage);
        //    string saveImagePath = DictionaryPath + "\\" + newFileName + exten;
        //    //save new image to file system.
        //    imgPhoto.Save(saveImagePath, ImageFormat.Png);
        //    imgPhoto.Dispose();
        //    File.Copy(saveImagePath, orgImage, true);
        ...
        /// 获取位置
        /// </summary>
        /// <param name="orgImage">原图对象</param>
        /// <param name="waterImage">水印图片对象</param>
        /// <param name="location">WaterMarkLocation值</param>
        /// <returns>int[]数组，int[0]为x,int[1]为y</returns>
        //private static int[] GetLoacation(Image orgImage, Image waterImage, WaterMarkLocation location)
        //{
        //    int[] loca=new int[2];
        //    int x = 0;
        //    int y = waterImage.Height;
        //    switch (location)
        //    {
        //        case WaterMarkLocation.Center:
        //            x = (orgImage.Width - waterImage.Width) / 2;
        //            y = (orgImage.Height - waterImage.Height) / 2;
        //            break;
        //        case WaterMarkLocation.LeftDown:
        //            x = 0;
        //            y = waterImage.Height;
        //            break;
        //        case WaterMarkLocation.LeftUp:
        //            x = 0;
        //            y = orgImage.Height;
        //            break;
        //        case WaterMarkLocation.RightDown:
        //            x = orgImage.Width - waterImage.Width;
        //            y = orgImage.Height - waterImage.Height;
        //            break;
        //        case WaterMarkLocation.RightUp:
        //            x = orgImage.Width - waterImage.Width;
        //            y = orgImage.Height;
        //            break;
        //        default:
        ...
        /// 缩小裁剪图片
        /// </summary>
        /// <param name="sWidth">裁剪后图片的宽度</param>
        /// <param name="sHeight">裁剪后图片的高度</param>
        /// <param name="startX">原图开始的X位置</param>
        /// <param name="startY">原图开始的Y位置</param>
        ...
        /// <param name="tempimg">临时图片路径</param>
        /// <returns></returns>
        public static bool ImgReduceCutOut(int sWidth, int sHeight, int startX, int startY, int int_Width, int int_Height, string input_ImgUrl, string out_ImgUrl, string tempimg)
        {
            try
            {
                // 上传标准图大小
                int int_Standard_Width = sWidth;
                int int_Standard_Height = sHeight;
                int level = 100; //缩略图的质量 1-100的范围


                //其实在图片裁剪过程中还可以传更多的参数,如把原始图片缩小了再进行裁剪.本实例中是裁剪后,再裁剪后的图片缩小成80X80
                //通过连接创建Image对象
                using (System.Drawing.Image oldimage = System.Drawing.Image.FromFile(input_ImgUrl))
                {
                    oldimage.Save(tempimg);//把原图Copy一份出来,然后在temp.jpg上进行裁剪,最后把裁剪后的图片覆盖原图
                    oldimage.Dispose();//一定要释放临时图片,要不之后的在此图上的操作会报错,原因冲突
                }
                Bitmap bm = new Bitmap(tempimg);
                //处理JPG质量的函数
                ...
                {
                    log.Error("裁剪头像，宽度：" + sWidth + ",保存地址：" + input_ImgUrl + "," + out_ImgUrl + "," + tempimg);
                    //保存图片
